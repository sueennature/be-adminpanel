  ...
  useEffect(() => {
    const fetchDashBoardData = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.BE_URL}/dashboard/daily_report/${dateStr}`,
          {
            headers: {
    ...
    const fetchDashBoardDataMonthly = async () => {
      try {
        const accessToken = Cookies.get("access_token");
        const response = await axios.get(
          `${process.env.BE_URL}/dashboard/monthly_report/${monthStr?.split("-")?.[0]}/${monthStr?.split("-")?.[1]}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${accessToken}`,
    ...
    <>
      {groupFour && (
        <>
          ...
          <h3 className="m-2 text-2xl font-bold text-black">Day Counts</h3>
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 md:gap-6 lg:grid-cols-4 2xl:gap-7.5">
            {Object.entries(dashboardDataDaly).map(
              ([status, value]: any, key) => (
                <Card key={key} title={status} data={value || {}}>
                  {" "}
                  <Calendar />
                </Card>
              ),
          ...
          <h3 className="m-2 text-2xl font-bold text-black">Monthly Counts</h3>
          <div className="mt-6 grid grid-cols-1 gap-4 md:gap-6 lg:grid-cols-4 2xl:gap-7.5">
            {Object.entries(dashboardDataMonthly).map(
              ([status, value]: any, key) => (
                <Card key={key} title={status} data={value}>
                  {" "}
                  <Calendar />
