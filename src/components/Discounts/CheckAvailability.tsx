...
const names = ["Single", "Deluxe", "Double", "Family", "Triple"];
...
const CheckAvailability = () => {
  ...
  const handleBooking = async () => {
    if (!personName || personName.length === 0 || views.length === 0 || !views) {
      console.log('seleted room', personName, 'views', views);
      toast.error("Please select both room type and room view");
    } else {
      try {
        setIsLoading(true);
            ...
            <div className="p-6.5">
              <div className="mb-6.5 flex flex-col gap-2 xl:flex-row">
                ...
                <div className="w-full xl:w-1/6">
                  <label className="mt- mb-3 block text-sm font-medium text-black">
                    Room View
                  </label>
                  <select
                    name="views"
                    ...
                    className="w-full rounded border-[1.5px] border-stroke bg-transparent px-2 py-3 text-xs text-black outline-none transition focus:border-primary active:border-primary"
                    required
                  >
                ...
                <div className="w-full xl:w-1/6">
                  ...
                  <input
                    ...
                    className="w-full rounded border-[1.5px] border-stroke bg-transparent px-2 py-3 text-xs font-normal text-black outline-none transition focus:border-primary active:border-primary"
                  />
                </div>

                <div className="w-full xl:w-1/6">
                  <button
                    //disabled={views === "" && selectedRoom === ""}
                    type="button"
                    onClick={handleBooking}
                    className="relative top-8 flex w-full justify-center text-nowrap rounded bg-primary p-3 text-xs font-medium text-gray"
                  >
                    {isLoading ? (
                      <CircularProgress
